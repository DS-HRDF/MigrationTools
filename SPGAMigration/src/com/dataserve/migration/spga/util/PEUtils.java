...
//        List<String> ccUsers = new ArrayList<String>();
        ...
        try {
            ...
            if (stepElement.hasParameterName("ExternalUnit")) {
                stepElement.setParameterValue("ExternalUnit", workitemObject.getExternalUnit(), true);
            }
...
//            reOrderInbasketColumns(workBasketDefinition, queueDefinition, c, applicationSpaceName);
    ...
    private VWWorkBasketColumnDefinition[] getWorkBasketColumns(VWWorkBasketDefinition workBasketDefinition, VWExposedFieldDefinition[] queueFields) {
        String[] allowedFields = new String[]{"ImportanceLevel:ImportanceLevel","Originator:Originator","UrgencyLevel:Urgency Level",
                "SequenceNumber:Sequence Number","F_Subject:Correspondence Subject","FromUser:From User","ExternalUnit:ExternalUnit",
                "ItemType:Item Type","ReceivedOnHijriDate:Received On HijriDate", "CorrespondenceDate:Correspondence Date",
                "HijricYear:HijricYear","isRead:isRead","MONumber:MONumber","ConfidentialityId:ConfidentialityId"};
...
//            fields.add(12, "Originator");
    ...
    public void createFilterDefinitions(VWWorkBasketDefinition workBasketDefinition, VWQueueDefinition queueDefinition){
        ...
        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("DisplayNumber");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("DisplayNumber");
            filterDefinition.setSearchField(queueDefinition.getField("DisplayNumber"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("DisplayNumber");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("UrgencyLevel");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("UrgencyLevel");
            filterDefinition.setSearchField(queueDefinition.getField("UrgencyLevel"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("UrgencyLevel");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("ImportanceLevel");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("ImportanceLevel");
            filterDefinition.setSearchField(queueDefinition.getField("ImportanceLevel"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("ImportanceLevel");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("LetterReturnedToAgency");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("LetterReturnedToAgency");
            filterDefinition.setSearchField(queueDefinition.getField("LetterReturnedToAgency"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("LetterReturnedToAgency");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("MONumber");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("MONumber");
            filterDefinition.setSearchField(queueDefinition.getField("MONumber"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("MONumber");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("ReceivedAction");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("ReceivedAction");
            filterDefinition.setSearchField(queueDefinition.getField("ReceivedAction"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("ReceivedAction");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("ReceivedAction");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("ConfidentialityId");
            filterDefinition.setSearchField(queueDefinition.getField("ConfidentialityId"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("ConfidentialityId");
        }

        filterDefinition = workBasketDefinition.getWorkBasketFilterDefinition("ExternalUnit");

        if(filterDefinition ==null){
            filterDefinition = workBasketDefinition.createWorkBasketFilterDefinition("ExternalUnit");
            filterDefinition.setSearchField(queueDefinition.getField("ExternalUnit"), VWWorkBasketFilterDefinition.OPERATOR_EQUAL);
            filterDefinition.setPrompt("ExternalUnit");
        }
    ...
    private VWSession getVwSession() {
        ...
        String connectionPoint = Utils.getConfigs("cp.prod");
        String uri = Utils.getConfigs("uri.prod"); //"cemp:iiop://10.50.21.53:9080/FileNet/Engine";//
