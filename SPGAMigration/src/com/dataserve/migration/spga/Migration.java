...
import com.dataserve.migration.spga.dao.*;
...
//                Attachment attachment = cedaoOld.getAttachment(data.getDocid());
                Attachment attachments = attachmentsDAO.getAttachment(data.getAttachmentPath(),data.getFileName(),data.getAttachmentSize());

                String migratedDocId = cedao.addDocument(data, attachments.getBytes(),attachments.getMimetype());
                ...
                logger.error(e.getMessage(), e);
                try {
                    dataDAO.updateStatus(data.getDocid(), Const.STATUS_ERROR, e.getMessage(), "",null);
                    data.setStatus(Const.STATUS_ERROR);
                    data.setMessage(e.getMessage());
                    Shared.spgaController.refreshData();
                } catch (Exception ex) {
